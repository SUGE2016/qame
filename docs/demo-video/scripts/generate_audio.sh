...
VOICE_ZH="$(python3 -c "import json; print(json.load(open('audio/voices.json'))['zh']['voice'])")"
VOICE_EN="$(python3 -c "import json; print(json.load(open('audio/voices.json'))['en']['voice'])")"
RATE_ZH="$(python3 -c "import json; print(json.load(open('audio/voices.json'))['zh']['rate'])")"
RATE_EN="$(python3 -c "import json; print(json.load(open('audio/voices.json'))['en']['rate'])")"
...
(root / "audio/timeline.json").write_text(json.dumps({"duration": round(t, 3), "scenes": timeline}, ensure_ascii=False, indent=2), encoding="utf-8")
(root / "scenes/timeline.js").write_text("window.QAME_TIMELINE = " + json.dumps({"duration": round(t, 3), "scenes": timeline}, ensure_ascii=False) + ";\n", encoding="utf-8")
